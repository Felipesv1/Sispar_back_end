    ...
    "specs": [
        {
            ...
            "route": "/apispec.json/", # <-- Rota do arquivo JSON para a contrução da documentação
            "rule_filter": lambda rule: True, # <-- Todas as rotas/endpoints serão documentados
            "model_filter": lambda tag: True ## <-- Especificar quais modelos da entidade serão documentados
        }
    ],
...
def create_app():
    app = Flask(__name__)
    CORS(app, origins="*") # Adicione a autorização do CORS para todas as rotas da aplicação
    app.register_blueprint(bp_colaborador)
    app.register_blueprint(bp_reembolso)
    app.config.from_object(Config) #Adiciona a configuração do arquivo de configuração
